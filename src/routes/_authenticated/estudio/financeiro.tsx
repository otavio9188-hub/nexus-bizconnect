...
import { ArrowDownCircle, ArrowUpCircle, Check, DollarSign, Pencil, Plus, Trash2, WalletCards, X } from "lucide-react";
...
import { createReceivable, deleteReceivable, listFinanceCustomers, listFinanceSuppliers, listReceivables, updateReceivable } from "@/lib/finance.functions";
...
type Form={description:string;amount:string;due_date:string;status:"PENDING"|"PAID"|"OVERDUE"|"CANCELLED";notes:string;customer_id:string;supplier_id:string;payment_date:string};
const empty=(kind:Kind):Form=>({description:"",amount:"",due_date:today(),status:"PENDING",notes:"",customer_id:"none",supplier_id:"none",payment_date:kind==="receivable"?today():today()});
...
function FinancePage(){
 ...
 const rf=useServerFn(listReceivables), pf=useServerFn(listPayables), cf=useServerFn(listFinanceCustomers), sf=useServerFn(listFinanceSuppliers);
 ...
 const rows=useMemo(()=>{const source=tab==="receivable"?rec:pay;return source.filter((x:any)=>{const q=search.toLowerCase();const match=!q||x.description.toLowerCase().includes(q)||(tab==="receivable"?(x.customers?.name||""):(x.suppliers?.name||x.commission_affiliate_name||"")).toLowerCase().includes(q);return match&&(status==="ALL"||x.status===status);});},[tab,rec,pay,search,status]);
 ...
 return <AppShell title="Financeiro" description="Contas a receber, contas a pagar e controle financeiro do Estúdio Nexus">
  <div className="space-y-6">
   ...
   <Card><CardHeader><CardTitle className="text-base">{tab==="receivable"?"Contas a receber":"Contas a pagar"} <span className="text-sm font-normal text-muted-foreground">({rows.length})</span></CardTitle></CardHeader><CardContent>{(r.isLoading||p.isLoading)?<p className="py-8 text-sm text-muted-foreground">Carregando...</p>:rows.length===0?<div className="py-10 text-center text-sm text-muted-foreground">Nenhum lançamento encontrado.</div>:<div className="space-y-2">{rows.map((x:any)=><div key={x.id} className="flex flex-col gap-3 rounded-lg border p-4 lg:flex-row lg:items-center"><div className="min-w-0 flex-1"><div className="font-medium">{x.description}</div><div className="text-xs text-muted-foreground">{tab==="receivable"?(x.customers?.name||"Sem cliente"):(x.suppliers?.name||"Sem fornecedor")} · vencimento {new Date(x.due_date+"T00:00:00").toLocaleDateString("pt-BR")}</div></div><div className="flex items-center gap-3"><Badge variant={statusVariant(x.status) as any}>{statusLabel(x.status)}</Badge><div className="min-w-28 text-right font-semibold">{money(Number(x.amount))}</div>{x.source==="commission"?<span className="text-xs text-muted-foreground">Gerenciar em Comissões</span>:<><Button size="icon" variant="ghost" onClick={()=>start(tab,x)} title="Editar"><Pencil/></Button><Button size="icon" variant="ghost" onClick={()=>remove(x)} title="Excluir"><Trash2/></Button></>}</div></div>)}</div>}</CardContent></Card>
  ...
  <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle>{editing?"Editar":"Novo"} {tab==="receivable"?"conta a receber":"conta a pagar"}</DialogTitle><DialogDescription>Preencha os dados financeiros. Valores pagos entram automaticamente no fluxo de caixa.</DialogDescription></DialogHeader>
   <div className="grid gap-4">
    ...
    {tab==="payable"&&<div><label className="mb-1 block text-sm font-medium">Fornecedor</label><Select value={form.supplier_id} onValueChange={v=>setForm({...form,supplier_id:v})}><SelectTrigger><SelectValue placeholder="Selecione o fornecedor"/></SelectTrigger><SelectContent><SelectItem value="none">Sem fornecedor</SelectItem>{(suppliers.data??[]).map((s:any)=><SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div>}
