...
import { CalendarDays, FilePlus2, Instagram, Facebook, Youtube, Clock3 } from "lucide-react";
...
function ContentPage(){
 ...
 return <AppShell title="Conteúdos" description="Calendário editorial e produção do Estúdio Nexus">
  <div className="space-y-6">
   ...
   <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{[["Total",rows.length,CalendarDays],["Rascunhos",status("DRAFT"),Clock3],["Agendados",status("SCHEDULED"),CalendarDays],["Publicados",status("PUBLISHED"),Instagram],["Aprovação",status("PENDING_APPROVAL"),FilePlus2]].map(([label,value,Icon])=><Card key={String(label)}><CardContent className="p-5"><Icon className="size-4 text-muted-foreground"/><div className="mt-3 text-2xl font-semibold">{String(value)}</div><div className="text-xs text-muted-foreground">{String(label)}</div></CardContent></Card>)}</div>
